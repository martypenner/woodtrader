...
EntityPlayer = ig.Entity.extend({
    ...
    draw: function () {
        ...
        if (!ig.global.wm) {
            ...
            this.fixZIndex(ig.game.getEntitiesByType('EntityTree'), 3);
        }
    ...
    // (ignoring sprite offset) than the player, stack the entity (visually)
    // above the player. Otherwise, stack it below the player. This makes
    // for a more realistic faux top-down RPG effect.
    fixZIndex: function (entities, relativeZIndex) {
        for (var i = 0; i < entities.length; i++) {
            entities[i].zIndex = (entities[i].pos.y) > this.pos.y
                ? this.zIndex + relativeZIndex
                : relativeZIndex;
        }
