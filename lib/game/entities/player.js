...
EntityPlayer = ig.Entity.extend({
    ...
    draw: function () {
        // Produce the in-front/behind effect for faux top-down RPGs (a la "A Link to the Past")
        this.fixZIndex(ig.game.getEntitiesByType('EntityTrader'), 1);
        this.fixZIndex(ig.game.getEntitiesByType('EntityStall'), 2);
    ...
    fixZIndex: function (entities, relativeZIndex) {
        // Loop through the given entities, checking player position versus
        // each entity's position. If the entity is lower on the screen
        // (factoring in offset) than the player, and the player's right edge
        // is within the entity's hitbox, move the entity (visually) above
        // the player. Otherwise, move it behind the player. This makes for a
        // more realistic faux top-down RPG effect

        var playerRightEdgeX = this.pos.x + this.size.x;

        for (var i = 0; i < entities.length; i++) {
            if (entities[i].pos.y - entities[i].offset.y > this.pos.y) {
                if (
                    playerRightEdgeX > entities[i].pos.x &&
                    playerRightEdgeX < entities[i].pos.x + entities[i].size.x
                ) {
                    entities[i].zIndex = this.zIndex + relativeZIndex;
                }
            } else {
                entities[i].zIndex = relativeZIndex;
            }
        }
