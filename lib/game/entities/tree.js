...
.defines(function () {
    ...
    EntityTree = ig.Creature.extend({
        ...
        // Preload sounds
        treeStrike: new ig.Sound(ig.CONFIG.PATH_TO_SOUNDS + 'tree-strike.*'),
        treeFall: new ig.Sound(ig.CONFIG.PATH_TO_SOUNDS + 'tree-fall.*'),
        ...
        kill: function () {
            ...
            if ((Math.random() * 100) < this.dropLogChance) {
                ...
                ig.game.spawnEntity(
                    EntityLog,
                    (this.size.x - 25) / 2 + this.pos.x,
                    (this.size.x - 25) / 2 + this.pos.y - 10
                );
            }
