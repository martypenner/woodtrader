...
ig.module('game.entities.tree').requires('plusplus.core.entity', 'game.common.flasher', 'plusplus.helpers.utils', 'plusplus.entities.particle-color', 'plusplus.abstractities.creature').defines(function() {
  var utils;

  utils = ig.utils;
  return ig.EntityTree = ig.Creature.extend({
    size: {
      x: 85,
      y: 55
    },
    ...
    kill: function() {
      ...
      if ((Math.random() * 100) < this.dropLogChance) {
        ig.game.spawnEntity(ig.EntityLog, (this.size.x - 25) / 2 + this.pos.x, (this.size.x - 25) / 2 + this.pos.y - 10);
      }
      return this.parent();
    }
  });
